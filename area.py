...
class Area:
    def __init__(self, N, name, net, wINHIPE, wEXCIPE, wINHIIR=None, wEXCIIR=None, IRPoisson=False, recordspikes=False):
        self.net = net
        self.name = name
        self.N = N
        ...
        if not IRPoisson:
            synapses(PPE, interIR, 'i==j', 35, net)
            synapses(NPE, IR, 'i==j', wEXCIPE, net)
...
def connect(a1, a2, W, wEXCIPE, wEXCIIR=None, plastic=False):
    ...
    if not a1.IRPoisson:
        synapses(net[a2.name+'_PPE'], net[a2.name+'_interIR'], (targets, sources), 35, net)
        synapses(net[a2.name+'_NPE'], net[a2.name+'_IR'], (targets, sources), wEXCIIR, net)
